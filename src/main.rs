...
lazy_static::lazy_static!{
    static ref REDIS_CLIENT: redis::Client = {
        let auth: Auth = toml::from_str(std::fs::read_to_string("./auth.toml").unwrap().as_str()).unwrap();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut rc = REDIS_CLIENT.get_connection()?;

    redis::cmd("set").arg("heartbeat").arg(chrono::Utc::now().timestamp()).query(&mut rc)?;

    tokio::task::spawn(async{ if let Err(e) = heartbeat() {println!("heartbeat() -> {}", e)} });
    tokio::task::spawn(async{ if let Err(e) = client_handler() {println!("client_handler() -> {}", e)} });
    tokio::task::yield_now().await;

    tokio::task::spawn(async{ if let Err(e) = test_client() {println!("test_client() -> {}", e)} });

    loop {
        let heartbeat: i64 = redis::cmd("get").arg("heartbeat").query(&mut rc).unwrap_or(0);
...
fn heartbeat() -> Result<(), Box<dyn std::error::Error>> {
    let mut rc = REDIS_CLIENT.get_connection()?;
    loop {
        redis::cmd("set").arg("heartbeat").arg(chrono::Utc::now().timestamp()).query(&mut rc)?;
        ...
        redis::cmd("publish").arg("clients:msg:all").arg(toml::to_string(&msg)?).query(&mut rc)?;

        redis::cmd("setnx").arg("reset").arg(true).query(&mut rc)?;
        let reset: bool = redis::cmd("get").arg("reset").query(&mut rc)?;
        if reset {
            println!("Beast database reset invoked.");
            redis::cmd("flushdb").query(&mut rc)?;
            redis::cmd("set").arg("reset").arg(false).query(&mut rc)?;
            redis::cmd("acl").arg("load").query(&mut rc)?;
            redis::cmd("set").arg("audit").arg(false).query(&mut rc)?;
        }

        redis::cmd("setnx").arg("audit").arg(true).query(&mut rc)?;
        let audit: bool = redis::cmd("get").arg("audit").query(&mut rc)?;
        if audit {
...
fn client_handler() -> Result<(), Box<dyn std::error::Error>> {
    let mut rc = REDIS_CLIENT.get_connection()?;
    ...
    loop {
        ...
        if msg.action == "auth_request" {
            ...
            redis::cmd("sadd").arg("clients").arg(&msg.from).query(&mut rc)?;
            redis::cmd("acl")
                .arg("setuser")
